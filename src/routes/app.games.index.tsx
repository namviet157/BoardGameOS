...
import { Boxes, Eye, MoreHorizontal, Plus, QrCode, RotateCcw, Search, Wrench } from "lucide-react";
  ...
  head: () => ({
    meta: [
      { title: "Kho game — BoardGameOS" },
      {
        name: "description",
        content:
          "Danh mục toàn bộ bộ board game của quán kèm trạng thái, vị trí lưu trữ và thao tác nhanh.",
      },
...
function GamesPage() {
  ...
  const hasFilters = Boolean(q || cat !== "all" || status !== "all" || players !== "all");
  const clearFilters = () => {
    setQ("");
    setCat("all");
    setStatus("all");
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-card p-3">
        ...
        <div className="mt-3 flex items-center justify-between border-t border-border pt-3">
          <p className="text-xs text-muted-foreground">
            Hiển thị <span className="font-semibold text-foreground">{filtered.length}</span> trong{" "}
            {games.length} game
          </p>
          {hasFilters ? (
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="Thử đổi từ khóa hoặc bỏ bớt bộ lọc để xem thêm kết quả."
        />
      ) : (
        <>
                  ...
                  <TableRow
                    ...
                    onKeyDown={(event) => {
                      if (event.key === "Enter" || event.key === " ") {
                        ...
                        navigate({ to: "/app/games/$gameId", params: { gameId: g.id } });
                      }
                    }}
                  >
                    <TableCell>
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              ...
                              variant="ghost"
                              className="h-8 w-8"
                              aria-label={`Thao tác với ${g.name}`}
                            >
                          ...
                          <DropdownMenuContent align="end" className="w-44">
                            ...
                            <DropdownMenuItem onSelect={() => setQrGame(g.id)}>
                              <QrCode /> Xem mã QR
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem
                              className="text-destructive focus:text-destructive"
                              onSelect={() => setMaintenanceGameId(g.id)}
                            >
                              <Wrench /> Chuyển bảo trì
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:hidden">
            {filtered.map((g) => (
              <Link
                ...
                to="/app/games/$gameId"
                params={{ gameId: g.id }}
                className="card-soft card-hover flex gap-3 p-4"
              >
                <GameThumb
                  emoji={g.emoji}
                  tone={g.tone}
        ...
        <DialogContent className="max-w-sm text-center">
          ...
          {qrTarget ? (
            <>
              <GameQrCode
                gameId={qrTarget.id}
                gameCode={qrTarget.code}
                gameName={qrTarget.name}
                size={176}
                showDownload
              />
              <div>
                <p className="font-medium">{qrTarget.name}</p>
                <p className="text-sm text-muted-foreground">Mã kho: {qrTarget.code}</p>
              </div>
            </>
          ) : null}
        </DialogContent>
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Nhập thông tin game và khai báo danh sách linh kiện chuẩn nếu đã có.
            </DialogDescription>
          </DialogHeader>
          <form className="grid gap-3 sm:grid-cols-2" onSubmit={submit}>
            ...
            <div className="space-y-2 sm:col-span-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Danh sách chuẩn để nhân viên đối chiếu khi nhận lại game.
                </p>
              </div>
            ...
            <DialogFooter className="sm:col-span-2">
              ...
              <Button type="submit" className="rounded-xl" disabled={imageProcessing}>
                Lưu game
              </Button>
            </DialogFooter>
          </form>
